...
from services import auth_service, user_service, video_service
import requests
...
def profile():
    ...
    try:
        ...
        profile["media_info"] = video_service.get_media_info(user)
...
def reset_password():
    try:
        ...
        if environ.get("FIREBASE_PROJECT_ID") is not None:
            ...
            user = auth.get_user_by_email(response["email"])
            current_app.logger.debug(
                "Successfully fetched user data: {0}".format(user.uid)
            )
            user = auth.update_user(user.uid, password=response["newPassword"])
            current_app.logger.debug(
                "Sucessfully updated user: uuid={0} - username={1} - email={2}".format(
                    user.uid, json_request["username"], response["email"]
                )
            )
